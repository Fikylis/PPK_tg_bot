...
# Dictionary mapping group names to their indices
GROUP_INDEX_MAPPING = {
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data in GROUPS:  # Это была кнопка с курсом
        ...
        # Получаем день параметр и текст даты сразу
        selected_date = query.data  # Это параметр, полученный из вашего fetch_dates

        # Здесь вам нужно получить текст даты из dates
        # Это может потребовать модификации вашего кода получения списка дат
...
async def fetch_schedule(group: str, day_param: str, selected_date: str) -> str:
    ...
    try:
        ...
        # Возвращаем расписание с правильной датой
